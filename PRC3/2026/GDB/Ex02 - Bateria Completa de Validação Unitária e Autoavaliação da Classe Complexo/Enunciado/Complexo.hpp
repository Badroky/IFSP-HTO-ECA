 ...
 * TEMA: CLASS para Números Complexos (C++)
 * ARQUIVO: Complexo.hpp
 *
 * CONCEITOS COBERTOS:
 * 1. União Discriminada (Tagged Union): Representação eficiente com alternância
 *    entre coordenadas Cartesianas (z = a + bi) e Polares (z = r ∠ θ).
 * 2. Forma Canônica Ortodoxa: Construtor padrão, cópia, atribuição e destrutor.
 * 3. Sobrecarga de Operadores: Aritmética mista, potenciação (^), I/O e comparadores.
 * 4. Funções Amigas (Friend/ADL): Padrão STL compatível com <cmath> e std::complex.
 * 5. Literais Definidos pelo Usuário: Sintaxe expressiva (ex.: 2.5_j, 45_deg).
 * ============================================================================= */
